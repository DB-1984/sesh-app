...
const deleteExerciseFromSesh = async (req, res) => {
  try {
    // 1. Grab both IDs from the URL params
    const { seshId, exerciseId } = req.params;

    // 2. Remove the reference from the Session using $pull
    // This is faster than fetching the whole doc and filtering manually
    const sesh = await Sesh.findByIdAndUpdate(
      seshId,
    ...
    // 3. Physical deletion of the actual Exercise document
    await Exercise.findByIdAndDelete(exerciseId);

    // 4. Return the freshly populated session to sync Redux
    const updatedSesh = await Sesh.findById(seshId).populate("exercises");
    res.status(200).json(updatedSesh);
  } catch (error) {
    console.error(`[Controller Error] deleteExerciseFromSesh: ${error.message}`);
    res.status(500).json({ message: "Server error" });
  }
};
